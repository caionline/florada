// ============================================================
// Florada — telas (parte 2): Avaliar, Comparar, Perfil
// ============================================================

// ---- ESCREVER AVALIAÇÃO ----------------------------------------------
function WriteReview({ presetStrain, go, addReview }) {
  const [strainId, setStrainId] = useState(presetStrain || STRAINS[0].id);
  const s = strainById(strainId);
  const [loteId, setLoteId] = useState(s.lotes[0].id);
  ...
  useEffect(() => { setLoteId(s.lotes[0].id); }, [strainId]);
  ...
  const submit = () => {
    addReview({
      id: "u" + Date.now(), strain: strainId, lote: loteId, author: "Você", initials: "VC",
    ...
    <div className="wrap" style={{ paddingTop: 60, paddingBottom: 80, maxWidth: 560, textAlign: "center" }}>
      <div className="fade-up card" style={{ padding: 44 }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <button className="btn btn-primary" onClick={() => go("strain", strainId)}>Ver a strain</button>
  ...
  return (
    <div className="wrap pad-bottom" style={{ paddingTop: 20, paddingBottom: 70, maxWidth: 720 }}>
      ...
      <h1 style={{ fontSize: 30, marginBottom: 6 }}>Avaliar uma strain</h1>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
            <label style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, color: "var(--ink-soft)" }}>Strain / cultivar</div>
              <select value={strainId} onChange={e => setStrainId(e.target.value)} style={fieldStyle}>
                {STRAINS.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}
              </select>
            </label>
            <label style={{ flex: 1, minWidth: 200 }}>
              <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, color: "var(--ink-soft)" }}>Lote / associação</div>
              <select value={loteId} onChange={e => setLoteId(e.target.value)} style={fieldStyle}>
                {s.lotes.map(l => { const a = assocById(l.assoc); return <option key={l.id} value={l.id}>{l.code} — {a.name} (THC {l.thc}%)</option>; })}
              </select>
            </label>
          </div>
        </div>
...
function CompareScreen({ go }) {
  ...
  const [strainId, setStrainId] = useState("charlotte");
  ...
  return (
    <div className="wrap pad-bottom" style={{ paddingTop: 24, paddingBottom: 70 }}>
      <SectionHead title="Comparar" sub="Coloque associações ou lotes lado a lado antes de escolher"/>
      <div style={{ display: "inline-flex", gap: 4, padding: 4, background: "var(--surface-2)", borderRadius: 999, marginBottom: 24 }}>
        {[["assoc","Associações"],["lotes","Lotes de uma strain"]].map(([k,l]) => (
          <button key={k} onClick={() => setMode(k)} style={{ padding: "8px 18px", borderRadius: 999, fontWeight: 600, fontSize: 14, background: mode === k ? "var(--surface)" : "transparent", color: mode === k ? "var(--ink)" : "var(--muted)", boxShadow: mode === k ? "var(--shadow-sm)" : "none" }}>{l}</button>
        ))}
      </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <div className="card" style={{ display: "grid", gridTemplateColumns: `170px repeat(${cols.length}, minmax(170px, 1fr))`, minWidth: 170 + cols.length * 170, overflow: "hidden" }}>
              ...
              {[
                ...
                ["Strains ofertadas", a => <span>{a.strains}</span>],
                ["Desde", a => <span className="mono">{a.since}</span>],
                ["Laudo em todo lote", a => a.verified ? <span style={{ color: "var(--primary)" }}><Icon name="check" size={18}/></span> : <span style={{ color: "var(--muted)" }}>—</span>],
              ].map(([label, render], ri) => (
      ...
      {mode === "lotes" && (
        <>
          <label style={{ display: "inline-block", marginBottom: 20 }}>
            <div className="label-cap" style={{ marginBottom: 7 }}>Strain</div>
            <select value={strainId} onChange={e => setStrainId(e.target.value)} style={{ ...fieldStyle, width: "auto", minWidth: 220 }}>
              {STRAINS.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}
            </select>
          </label>
          <div style={{ overflowX: "auto" }}>
            <div className="card" style={{ display: "grid", gridTemplateColumns: `150px repeat(${s.lotes.length}, minmax(160px, 1fr))`, minWidth: 150 + s.lotes.length * 160, overflow: "hidden" }}>
              <CompCell head label=""/>
              {s.lotes.map(l => { const a = assocById(l.assoc); return <CompCell head key={l.id} label={<div><div className="mono" style={{ fontWeight: 700, fontSize: 14 }}>{l.code}</div><div style={{ fontSize: 12, color: "var(--muted)", fontWeight: 500 }}>{a.name}</div></div>}/>; })}
              {[
                ["THC", l => <span className="mono">{l.thc}%</span>],
                ["CBD", l => <span className="mono">{l.cbd}%</span>],
                ["Razão CBD:THC", l => <span className="mono">{(l.cbd/Math.max(l.thc,0.1)).toFixed(1)}:1</span>],
                ["Colheita", l => <span>{l.date}</span>],
                ["Terpenos", l => <span style={{ fontSize: 12.5 }}>{l.terps.join(", ")}</span>],
                ["Laudo", l => l.verified ? <span style={{ color: "var(--primary)", display: "inline-flex", alignItems: "center", gap: 4, fontSize: 12.5, fontWeight: 600 }}><Icon name="shield" size={15}/> ok</span> : <span style={{ color: "var(--muted)" }}>—</span>],
              ].map(([label, render], ri) => (
                <React.Fragment key={label}>
                  <CompCell rowLabel zebra={ri%2===1} label={label}/>
                  {s.lotes.map(l => <CompCell key={l.id} zebra={ri%2===1} label={render(l)}/>)}
                </React.Fragment>
              ))}
            </div>
          </div>
...
function ProfileScreen({ go, favs, userReviews, toggleFav }) {
  ...
  const savedStrains = STRAINS.filter(s => favs.has(s.id));
...
Object.assign(window, { WriteReview, CompareScreen, ProfileScreen });
